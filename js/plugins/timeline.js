const StellarTimeline = {
  ...
  dynamicColor: (colors) => {
    ...
    colors.forEach((hex_color, i) => {
      ...
      let dark_text_color = `hsl(${h}deg, 80%, 72%)`;
      style_light += `--timeline-label-bg-${base_index + i}: ${light_bg};--timeline-label-text-color-${base_index + i}: ${light_text_color};`;
      styel_dark += `--timeline-label-bg-${base_index + i}: ${dark_bg};--timeline-label-text-color-${base_index + i}: ${dark_text_color};`;
    })
    let style_str = `:root {${style_light}}@media (prefers-color-scheme: dark) {:root:not([color-scheme]) {${styel_dark}}}[color-scheme='dark']{${styel_dark}}`;
