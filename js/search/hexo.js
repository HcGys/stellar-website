...
`;
  fn.init = async () => {
    ...
    fn.modal = document.querySelector("#u-search .modal");
    fn.modal_overlay = document.querySelector("#u-search .modal-overlay");
  };
  fn.onSubmit = (event) => {
  ...
  fn.search = async () => {
    ...
    util.animateIn(fn.modal, "slide-in .5s");
    util.animateIn(fn.modal_overlay, "to_show 0.5s");
    ...
    if (results === "") {
      results = `<div id="resule-hits-empty"><i class="fa-solid fa-box-open"></i><p>${stellar.GLOBAL_CONFIG.languages.search.hits_empty.replace(/\$\{query}/, fn.queryText)}</p></div>`
    }
  ...
  fn.close = () => {
    util.animateOut(fn.modal, "slide-out .5s");
    util.animateOut(fn.modal_overlay, "to_hide 0.5s");
    // document.querySelector("#u-search").style.display = "none";
    document.querySelector('body').removeAttribute('style');
  };
  fn.fetchData = () => {
    return fetch(stellar.GLOBAL_CONFIG.search.dataPath)
