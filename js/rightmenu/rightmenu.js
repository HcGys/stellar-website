const RightMenus = {
    defaultEvent: ['copyText', 'copyLink', 'copyPaste', 'copyAll', 'copyCut', 'copyImg', 'printMode', 'readMode', 'prev', 'next','toFullscreen', 'toOrign'],
    defaultGroup: ['navigation', 'inputBox', 'seletctText', 'elementCheck', 'elementImage', 'articlePage', 'prevNext', 'elementFullscreen'],
  ...
  RightMenus.fun = (() => {
    ...
    const
      ...
      _fullWayFlag = document.querySelector('article.md-text.sa-load-hidden, article.md-text.blur'),  // 包含这些样式时会导致fixed（要全屏元素的布局）相对mdText（文章容器）定位而不是整个页面，从而出现冲突，此时需要选择方案二来进行页面全屏切换
      _bodyDiv = document.querySelector('body');
        ...
        if (globalData.statusCheck || globalData.isArticle || globalData.isWiki) {
          switch (groupName) {
            ...
            case 'elementFullscreen':
              if (globalData.FS.isAccess) {
                let full_item = document.querySelector('.menuLoad-Content #fullscreen');
                let no_full_item = document.querySelector('.menuLoad-Content #no-fullscreen');

                no_full_item.parentElement.style.display = globalData.FS.target.parentElement.classList.contains('fullscreen') ? 'block' : 'none';
                full_item.parentElement.style.display = !globalData.FS.target.parentElement.classList.contains('fullscreen') ? 'block' : 'none';
              }
              break;
            default:
    ...
    fn.globalDataSet = (event) => {
      ...
      // 判断是否具有全屏显示的功能
      var node = fn.findParent(event.target, 'fullscreen-access', 'body-wrapper');
      globalData.FS.target = node;
      globalData.FS.eventFunc = null;
      globalData.FS.nextCallback = null;
      if (node) {
        globalData.FS.isAccess = true;
        if (node.classList.contains('mermaid')) {
          globalData.FS.eventFunc = (fullDiv)=>{
            let ele = globalData.FS.target.cloneNode(true);
            fullDiv.appendChild(ele);

            ele.removeAttribute('data-processed');
            ele.innerHTML = ele.getAttribute('data-original-code');
            mermaid.run({
              suppressErrors: true,
              nodes: [ele],
            });
          };
        } else if (node.classList.contains('echarts-box')) {
          globalData.FS.eventFunc = (fullDiv)=>{
            let template = '<div id="echarts_copy" class="echarts" style="width: 100%; height: 100%;" ></div></div>';
            let echarts_box = document.createElement('div');
            echarts_box.classList.add('echarts-box', 'fullscreen-access');
            echarts_box.innerHTML = template;

            fullDiv.appendChild(echarts_box);

            let eNode = echarts_box.firstElementChild;
            echarts_contents['echarts_copy'] = echarts_contents[node.firstElementChild.id];

            echarts_render_obverser.observe(eNode);
          };
          globalData.FS.nextCallback = (ele)=>{
            echarts.getInstanceByDom(ele.firstElementChild).dispose();
            ele.firstElementChild.classList.remove('drew');
          };
        }
      } else {
        globalData.FS.isAccess = false;
      }
    ...
    fn.contextmenu = () => {
      window.document.oncontextmenu = (event) => {
        ...
        stellar.dom.mask.style.display = 'block';
    ...
    fn.copyText = () => {
      util.writeClipText(globalData.selectText)
        .then(() => {
          if (RightMenus.messageRightMenu) {
    ...
    fn.copyLink = () => {
      util.writeClipText(globalData.linkUrl || globalData.mediaLinkUrl)
        .then(() => {
          if (RightMenus.messageRightMenu) {
            util.messageCopyright();
          }
    ...
    fn.findParent = (ele, findSelector, stopSelector) => {
      while(!ele.classList.contains(findSelector)) {
        if (!ele.parentElement || ele.classList.contains(stopSelector)) return null;
    ...
    fn.toFullscreen = () => {
      ...
      if (!_fullWayFlag) {
        ...
        stellar.requestAnimationFrame(()=>{
          var oldEle = globalData.FS.oldTarget;
          var eleEvent = globalData.FS.eventFunc;
          var callback = globalData.FS.callback;
          var nextCallback = globalData.FS.nextCallback;
          ...
          if (eleEvent) {
            ...
            let fullEle = ele.cloneNode(true);
            fullDiv.appendChild(fullEle);
          }

          _bodyDiv.appendChild(fullDiv);
          _bodyDiv.setAttribute('style', 'overflow:hidden;');

          util.animateIn(fullDiv, "slide-in .5s");

          if (callback) {
            callback(oldEle);
          }
          globalData.FS.callback = nextCallback;
        });
      } 
    }
    ...
    fn.toOrign = () => {
      ...
      if (!_fullWayFlag) {
        ...
        // 方案二：用于启用scroolanimate插件或模糊背景的时候
        stellar.requestAnimationFrame(()=>{
          _bodyDiv.removeAttribute('style');
          util.animateOut(ele.parentElement, "slide-out .5s");
          globalData.FS.oldTarget = ele;
        });
      }
    }
