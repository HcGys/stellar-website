      ...
      element.addEventListener('click', event => {
        ...
        if (stellar.plugins.mermaid) {
          ...
          if (mermaidFirstNode && !mermaidFirstNode.classList.contains('drew')) {
            ...
            mermaidNodes.forEach(m=>{m.classList.add('drew')});
          }
        }
        if (stellar.plugins.echarts) {
...
if (stellar.plugins.mermaid) {
  var first_render = [];
  var memad = document.querySelectorAll('.mermaid');
  for (var i = 0; i < memad.length; ++i) {
    if (!memad[i].classList.contains('intab')) {
      first_render.push(memad[i]);
    }
  }
  mermaid.run({
    suppressErrors: true,
